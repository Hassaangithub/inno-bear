...
import {allUserChallenges} from '../Services/dashboard';
import {toast, ToastContainer} from 'react-toastify';

const AllCurrentChallenges = () => {
  ...
  useEffect(() => {
    const getChallenges = async () => {
      setLoading(true);
      const response = await allUserChallenges();
      if (!response) {
        toast.error(response.response.data.message);
      } else {
        setData(response.data.successData.userChanllenges);
        setLoading(false);
      }
    };
    getChallenges();
...
export default AllCurrentChallenges;
